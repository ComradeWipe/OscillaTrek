...
/**
  ******************************************************************************
  * @file           : main.c
  * @brief          : Main program body for STM32F401RE
  ******************************************************************************
  * @attention
  *
...
UART_HandleTypeDef huart1; // Added for USART1
...
static void MX_USART1_UART_Init(void); // Added for USART1
static void MX_USART2_UART_Init(void);
static void MX_TIM2_Init(void);

/* USER CODE BEGIN PFP */
// Function to send uint8_t data via USART1
void USART1_SendUint8(uint8_t data);
...
// Redirect printf to USART2 for debugging
...
int main(void)
{
  ...
  // Test USART2 for debugging
  printf("USART2 Initialized for Debugging\r\n");
  /* USER CODE END 2 */
  ...
  while (1)
  {
    ...
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_5, GPIO_PIN_SET); // Set Trig1 high
    delay_us(10); // Delay 10 microseconds
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_5, GPIO_PIN_RESET); // Set Trig1 low
    ...
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_9, GPIO_PIN_SET); // Set Trig2 high
    delay_us(10); // Delay 10 microseconds
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_9, GPIO_PIN_RESET); // Set Trig2 low
    ...
    // Send average_distance via USART1
    USART1_SendUint8(average_distance);
...
void SystemClock_Config(void)
{
  ...
  // Adjust this function based on your project settings and hardware
  ...
  RCC_OscInitStruct.PLL.PLLM = 8; // PLLM value (depends on your HSE frequency)
...
{
  /* Configure USART1 for communication at 115200 baud rate */
  ...
  huart1.Init.Mode = UART_MODE_TX; // Enable transmit only
...
static void MX_USART2_UART_Init(void)
{
  /* Configure USART2 for debugging at 115200 baud rate */
...
static void MX_GPIO_Init(void)
{
  ...
  /* Configure GPIO pin: PB6 (USART1_TX) */
  GPIO_InitStruct.Pin = GPIO_PIN_6;
  GPIO_InitStruct.Mode = GPIO_MODE_AF_PP; // Alternate Function Push-Pull
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_VERY_HIGH; // Very high speed for USART
  GPIO_InitStruct.Alternate = GPIO_AF7_USART1; // Set alternate function to USART1_TX
  HAL_GPIO_Init(GPIOB, &GPIO_InitStruct);
}
